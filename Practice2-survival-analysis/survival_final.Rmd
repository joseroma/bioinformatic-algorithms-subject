...
```{r echo=FALSE}
load(file="savedData/groupingsData.RData")
```
...
```{r echo=FALSE}
load(file="savedData/aproxim.RData")
```
...
```{r echo=FALSE}
load(file="savedData/recursivefinding.RData")
```
...
```{r echo=FALSE}
df.edad.norm<-data.edad.group
df.edad.norm$tgrade<-factor(data.grado.group$tgrade)
df.edad.norm$pnodes<-factor(data.nodes.ranges$pnodes)
df.edad.camb<-data.edad.group1
df.edad.camb$tgrade<-factor(data.grado.group$tgrade)
df.edad.camb$pnodes<-factor(data.nodes.ranges$pnodes)

df.edad.camb$tsize<-cut(as.numeric(df.edad.camb$tsize), breaks = c(0,25,50,75,100))
df.edad.norm$tsize<-cut(as.numeric(df.edad.norm$tsize), breaks = c(0,25,50,75,100))

df.edad.norm$age<-factor(df.edad.norm$age)
df.edad.norm$menostat<-factor(df.edad.norm$menostat)
df.edad.norm$tsize<-factor(df.edad.norm$tsize)
```
...
```
```{r echo=FALSE}
#save(data.fill, data.trunc, file="savedData/datafiltered.RData")
load(file="savedData/datafiltered.RData")
```
...
cdat.tcga.select<-subset(cdat.tcga, select = c(CASE_ID, AGE, AJCC_PATHOLOGIC_TUMOR_STAGE,AJCC_TUMOR_PATHOLOGIC_PT , MENOPAUSE_STATUS, LYMPH_NODE_EXAMINED_COUNT,  IHC_HER2, ER_STATUS_BY_IHC, PR_STATUS_BY_IHC, OS_MONTHS, OS_STATUS))
...
data.inc.phen$AGE<-as.numeric(data.inc.phen$AGE)
data.inc.phen$LYMPH_NODE_EXAMINED_COUNT<-as.numeric(data.inc.phen$LYMPH_NODE_EXAMINED_COUNT)

data.inc.phen<-data.inc.phen[!(data.inc.phen$LYMPH_NODE_EXAMINED_COUNT %in% c("",NA)),]

data.inc.phen$MENOPAUSE_STATUS<-factor(data.inc.phen$MENOPAUSE_STATUS)
levels(data.inc.phen$MENOPAUSE_STATUS) <- c('1', '2', '3','Post' , 'Pre')
data.inc.phen<-data.inc.phen[!(data.inc.phen$menostat %in% c("1","2","3")),]

# Renombramos columnas del data.frame
names(data.inc.phen)<-c('ID', 'age', 'tumorState', 'pathologicPT', 'menostat', 'nodesCount', 'HER2','ER','PR', 'time','cens','Pheno')
#data.truc <- data.inc.phen [ , c(1:8,11,9,10)]
data.reviewed.trunc<-data.inc.phen
...
data.inc.phen$AGE<-as.numeric(data.inc.phen$AGE)
data.inc.phen$LYMPH_NODE_EXAMINED_COUNT<-as.numeric(data.inc.phen$LYMPH_NODE_EXAMINED_COUNT)
data.inc.phen$LYMPH_NODE_EXAMINED_COUNT<-replace(data.inc.phen$LYMPH_NODE_EXAMINED_COUNT, is.na(data.inc.phen$LYMPH_NODE_EXAMINED_COUNT), round(mean(data.inc.phen$LYMPH_NODE_EXAMINED_COUNT, na.rm=TRUE)))
...
# Renombramos columnas del data.frame
names(data.inc.phen)<-c('ID', 'age', 'tumorState', 'pathologicPT', 'menostat', 'nodesCount', 'HER2','ER','PR', 'time','cens','Pheno')
data.reviewed.fill<-data.inc.phen
...
data.reviewed.fill$nodesCount <- cut(as.numeric(data.reviewed.fill$nodesCount), breaks = c(0,3,5,100))
data.reviewed.trunc$nodesCount <- cut(as.numeric(data.reviewed.trunc$nodesCount), breaks = c(0,3,5,100))

#save(data.reviewed.fill, data.reviewed.trunc, file="savedData/correct_data_sets.RData" )
```
```{r echo=FALSE}
load(file="savedData/correct_data_sets.RData")
```
...
```{r asegurar, message=FALSE, warning=FALSE}
fit.nodos.IHC.fill<-survfit(Surv(data.fill$time, data.fill$cens) ~ data.fill$nodesIHC, data=data.fill) 
fit.nodos.IHC.trunc<-survfit(Surv(data.trunc$time, data.trunc$cens) ~ data.trunc$nodesIHC, data=data.trunc) 
fit.nodos.HE.fill<-survfit(Surv(data.fill$time, data.fill$cens) ~ data.fill$nodesHE, data=data.fill) 
fit.nodos.HE.trunc<-survfit(Surv(data.trunc$time, data.trunc$cens) ~ data.trunc$nodesHE, data=data.trunc)
...
```{r echo=FALSE}
data.edad.group1p<-GBSG2
colnames.trun.p1<-colnames(data.edad.group1p)
colnames.trun.p1[2]<-"stratAge"
colnames(data.edad.group1p)<-colnames.trun.p1
data.edad.group1p$age <- cut(as.numeric(data.edad.group1p$stratAge), breaks = c(0,45,63,100))  #Creando nueva variable

op1<-list()
fit.edad.opt1<-survfit(Surv(time,cens) ~ age, data=data.edad.group1p) 
op1[[1]]<-ggsurvplot(fit.edad.opt, conf.int = TRUE, censor= TRUE, cex.axis=3, cex.lab=3.0, main="Survival curve Age grouping", pval = TRUE)
op1[[2]]<-ggsurvplot(fit.edad.opt1, conf.int = TRUE, censor= TRUE, cex.axis=3, cex.lab=3.0, main="Survival curve Age grouping", pval = TRUE)

arrange_ggsurvplots(op1, print = TRUE, ncol =2, nrow = 1)
```
...
```{r echo=FALSE}
load(file="savedData/sin.RData")
dat.fil.sin$ID<-NULL
dat.fil.sin$stratAge<-NULL
dat.fil.sin$tumorState<-NULL
dat.fil.sin$pathologicPT<-NULL
dat.fil.sin$ER<-NULL
dat.fil.sin$PR<-NULL
dat.fil.sin$HER2<-NULL

dat.fil$ID<-NULL
dat.fil$stratAge<-NULL
dat.fil$tumorState<-NULL
dat.fil$pathologicPT<-NULL
dat.fil$ER<-NULL
dat.fil$PR<-NULL
dat.fil$HER2<-NULL

dat.trun.sin$ID<-NULL
dat.trun.sin$stratAge<-NULL
dat.trun.sin$tumorState<-NULL
dat.trun.sin$pathologicPT<-NULL
dat.trun.sin$ER<-NULL
dat.trun.sin$PR<-NULL
dat.trun.sin$HER2<-NULL

dat.trun$ID<-NULL
dat.trun$stratAge<-NULL
dat.trun$tumorState<-NULL
dat.trun$pathologicPT<-NULL
dat.trun$ER<-NULL
dat.trun$PR<-NULL
dat.trun$HER2<-NULL

dat.fil<-dat.fil[,c("age", "menostat", "nodesCount", "Pheno", "time", "cens")]
dat.fil.sin<-dat.fil.sin[,c("age", "menostat", "nodesCount", "Pheno", "time", "cens")]
dat.trun.sin<-dat.trun.sin[,c("age", "menostat", "nodesCount", "Pheno", "time", "cens")]
dat.trun<-dat.trun[,c("age", "menostat", "nodesCount", "Pheno", "time", "cens")]
```
